...
bool TSPSolver::solve ( const TSP& tsp , const TSPSolution& initSol , TSPSolution& bestSol )
{
  try
  {
    ...
    TSPMove move;
    while ( ! stop ) {
      double bestDecrement = findBestNeighbourDecrement(tsp, currSol, move);
...
double TSPSolver::findBestNeighbourDecrement(const TSP& tsp, const TSPSolution& currSol, TSPMove& move){
